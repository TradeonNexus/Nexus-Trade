...
export default function DepositPage() {
  ...
  const [selectedToken, setSelectedToken] = useState<Token>("TUSDC")
  ...
  const handleDeposit = () => {
    ...
    setTimeout(() => {
      ...
      if (wallet?.balance) {
        wallet.balance[selectedToken] = (wallet.balance[selectedToken] || 0) + Number(amount)
        localStorage.setItem("nexus_wallet", JSON.stringify(wallet))
      }

      toast({
        title: "Deposit Successful",
        description: `Successfully deposited ${amount} ${selectedToken} to your wallet.`,
      })
    ...
    <AuthenticatedLayout>
      <div className="p-6 flex flex-col items-center justify-center min-h-[calc(100vh-4rem)] bg-gradient-to-b from-black to-[#001020]">
        ...
        >
          ...
          <div className="mb-6">
            <label className="block text-sm text-gray-400 mb-2">Amount to be deposited</label>
          ...
          <div className="mb-6">
            <div className="flex space-x-2">
              <button
                ...
                onClick={() => setSelectedToken("TUSDC")}
              >
                TUSDC
              </button>
            </div>
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
          >
            {isLoading ? "Processing..." : `Deposit ${amount || "0.25"}`}
          </motion.button>
        </motion.div>
      </div>
    </AuthenticatedLayout>
